...
$args = array(
    ...
    'posts_per_page' => -1,
    'meta_key' => 'schedule_start_date', // Replace with the actual ACF field name
    'orderby' => 'meta_value',
    'order' => 'DESC',
    'meta_query' => array(
        array(
    ...
    <main id="main" class="page-main site-main" role="main">
        <section class="s-products wrapper">
            <div class="container inner">
                ...
                <h1 class="title heading wrapper wrapper_size_xl">Events</h1>
            </div>
            <svg xmlns="http://www.w3.org/2000/svg" width="1920" height="433" fill="none" class="background-image">
                ...
                <g mask="url(#a)">
                    ...
                    <path fill="#E9EBF2"
                        d="M115.276 185.187s-8.82-3.042-4.642-10.962l10.621-10.721 8.81 5.657-14.789 16.026Z" />
                    <path fill="#fff"
            ...
            <div class="container">
                ...
                <div class="events-list__years-wrapper">
                    ...
                    foreach( $events_years as $event_year ): 
                        $active_class = ($queried_year == $event_year) ? 'active' : '';
                        $page_link = ($event_year == $current_year) ? 'events/' : 'past-events/';
                        $append_year = ($event_year == $current_year) ? '' : $event_year;
                        ?>
                        <a href="<?php echo pll_home_url() . $page_link . $append_year; ?>" class="events-list__year <?php echo $active_class; ?>"><?= $event_year; ?></a>
                    <?php
                    endforeach; ?>
                </div>
                <?php
                ...
                $query_events_list = new WP_Query( $args );

                if( $query_events_list->have_posts() ):
                ?>
                <ul class="events-list__items-wrapper">
                    <?php
                    while( $query_events_list->have_posts() ): $query_events_list->the_post(); 
